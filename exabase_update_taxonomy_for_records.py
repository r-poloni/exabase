...
def insert_taxonomy(usageKey, order, family, subfamily, tribe, genus, subgenus, species, canonicalName, authorship, scientificName, rank, isInGBIF):
    query = f"""
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)
        """
    values = (usageKey, order, family, subfamily, tribe, genus, subgenus, species, canonicalName, authorship, scientificName, rank, '1')
    cursor = connection.cursor()
...
with open("records_example_data.csv", mode="r", newline='', encoding="utf-8") as file:
    ...
    next(reader, None)
    for row in reader:
        usagekey = get_species_gbif_usage_key(scientific_name(row))
        if usagekey != '':
            ...
            if usage_key_in_taxonomy is None:
                ...
                species = from_gbif[6]
                canonicalName = scientific_name(row)
                authorship = from_gbif[8]
                scientificName = from_gbif[9]
                rank = from_gbif[10]
                isInGBIF = from_gbif[11]
                insert_taxonomy(usagekey, order, family, subfamily, tribe, genus, subgenus, species, canonicalName, authorship, scientificName, rank, isInGBIF)
            else:
                print("usagekey already present")
        else:
            usagekey_tmp = scientific_name(row)
